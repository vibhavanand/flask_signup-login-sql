...
def login():
    if request.method == 'POST':
        ...
        query = session.query(User).filter(
            User.username.in_([user_name]), User.password.in_([pwd]))
        q = query.first()
        ...
        # if verified, creating an access token,updating it in the DB to verify it in future and passing it to the client.
            ...
            # first encoding the username pwd, then converting the byte object to string
            authtoken = encode_credentials_jwt(user_name, pwd).decode("utf-8")
            session.query(User).filter(User.username.in_(['adminn'])).update(
                {'authentication_token': authtoken}, synchronize_session=False)
            session.commit()
...
def auth_token():
    if request.method == 'POST':
        ...
        query = session.query(User).filter(User.username.in_([user_name]), User.password.in_(
            [pwd]), User.authentication_token.in_([access_token_client]))
        q = query.first()
        if query is None:
